...
void notePlayClassic(uint32_t frequence, float duree)
{
	...
	for(uint32_t k = 0; k < calculNbEchNote(duree); k++) {
		TIME_START;
		echOutputLeft = (int16_t) AMPLITUDE * sin((2*PI*k)/nbEchPeriod);
		echOutputRight = (int16_t) AMPLITUDE * sin((2*PI*k)/nbEchPeriod);
		TIME_STOP;
...
void initSinusTable(void)
{
	for(uint32_t i = 0; i < BUFFER_SIZE_SINUS; i++) {
		sinusTable[i] = (int16_t) AMPLITUDE * sin((2*PI*i)/AUDIOFREQ_16K);
	}
}
...
{
	uint32_t nbEchPeriod = calculNbEchPeriod(frequence);
	uint32_t nbEchEcart = AUDIOFREQ_16K / nbEchPeriod;
	for(uint32_t k = 0; k < calculNbEchNote(duree); k++) {
		TIME_START;
		echOutputLeft = (int16_t) sinusTable[(k * nbEchEcart) % AUDIOFREQ_16K];
		echOutputRight = (int16_t) sinusTable[(k * nbEchEcart) % AUDIOFREQ_16K];
		TIME_STOP;
...
{
	float a1 = (sin((2*2*PI*frequence)/AUDIOFREQ_16K)/sin((2*PI*frequence)/AUDIOFREQ_16K));
	float a2 = 1;

	audioTable[BUFFER_SIZE_SINUS-1] = 1;
	audioTable[BUFFER_SIZE_SINUS-2] = audioTable[BUFFER_SIZE_SINUS-1] * -a1;
	audioTable[BUFFER_SIZE_SINUS-3] = audioTable[BUFFER_SIZE_SINUS-2] * -a1 + audioTable[BUFFER_SIZE_SINUS-1] * -a2;

	for(uint32_t k = 2; k < calculNbEchNote(duree); k++) {
		TIME_START;
		echOutputLeft = (int16_t) audioTable[(BUFFER_SIZE_SINUS-k-1) % BUFFER_SIZE_SINUS] * -a1 + audioTable[(BUFFER_SIZE_SINUS-k-2) % BUFFER_SIZE_SINUS] - -a2;
		echOutputRight = (int16_t) echOutputLeft;
		TIME_STOP;

		//Vieillissement sortie
		audioTable[BUFFER_SIZE_SINUS-k] = echOutputLeft;
...
int main(void)
{
	...
	*/
		  ...
		  //notePlayClassic(200, 0.3);
		  //notePlayClassic(480, 0.45);
		  //notePlayClassic(650, 0.3);
