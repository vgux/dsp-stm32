...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define FREQ_ECH_48000		48000
#define FREQ_ECH_16000		16000
...
int main(void)
{
  ...
  uint16_t oldestIndex = BUFFER_SIZE_INPUT - 2;
  while (1)
  {
	  //passThrough();
	...
	/*
		// Reception des échantillons d'entrée
		HAL_SAI_Receive (&hsai_BlockB2,(uint8_t *)&bufferInputLeft[index],1,SAI_WAIT);
		HAL_SAI_Receive (&hsai_BlockB2,(uint8_t *)&bufferInputRight[index],1,SAI_WAIT);

		//float32_t


		// Calcul du filtre (ici Out = In)
		echOutputLeft = bufferInputLeft[index] * 0.9 + bufferInputLeft[(index - 1) % BUFFER_SIZE_INPUT] * 0.9;
		echOutputRight = bufferInputRight[index] * 0.9 + bufferInputRight[(index - 1) % BUFFER_SIZE_INPUT] * 0.9;
	  ...
	  if(oldestIndex == 0) {
		  oldestIndex = BUFFER_SIZE_INPUT - 1;
	  } else {
		  oldestIndex--;
	  }
*/
	  ...
	  // Reception des échantillons d'entrée
	  		...
	  		// Calcul du filtre (ici Out = In)
	  		echOutputLeft = bufferInputLeft[index] + bufferOutputLeft[(index - 1) % BUFFER_SIZE_INPUT] * 0.6;
	  		echOutputRight = bufferInputRight[index] + bufferOutputRight[(index - 1) % BUFFER_SIZE_INPUT] * 0.6;
...
void SystemClock_Config(void)
{
  ...
  */
  ...
  PeriphClkInitStruct.PeriphClockSelection = RCC_PERIPHCLK_USART1|RCC_PERIPHCLK_SAI2
                              |RCC_PERIPHCLK_I2C3;
  PeriphClkInitStruct.PLLI2S.PLLI2SN = 71;
